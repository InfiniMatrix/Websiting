    ...
    function loadLanguage(langCode) {
        ...
        const langPromise = new Promise((resolve, reject) => {
            fetch(`/assets/lang/${langCode}.json`)
                .then(response => {
                    if (!response.ok) {
                        throw new Error(`Failed to load language file for ${langCode}`);
                    }
                    return response.json();
                })
                .then(data => {
                    defaultStrings[langCode] = data;
...
const Header = (function() {
    ...
    function setupLanguageDropdowns() {
        ...
        document.querySelectorAll('#lang-menu a, #mobile-lang-menu a').forEach(link => {
            ...
            link.addEventListener('click', function(e) {
                ...
                // Try setting the language using I18n module
                I18n.setLanguage(langCode)
                    .then(() => {
                        ...
                        // Try the AJAX content loading approach first
                        return fetchLanguageContent(newUrl, langCode, pageState)
                            .catch(error => {
                                console.warn('AJAX language content loading failed, falling back to client-side translation:', error);
                                // Just rely on client-side translation if AJAX fails
                    ...
                    .then(() => {
                        ...
                        setTimeout(() => {
                            ...
                            Accessibility.announce(I18n.get('languageSwitchedTo', { language: CONFIG.supportedLanguages.find(l => l.code === langCode)?.name || langCode }));
                        }, 300);
                    })
                    .catch(error => {
    ...
    /**
     * Show error message when language switching fails
     */
    function showLanguageSwitchError(langCode) {
        ...
        messageContainer.innerHTML = `
            ...
            <p class="mb-4">${I18n.get('languageLoadError', { language: langName })}</p>
            <div class="flex justify-center gap-2">
                <button id="language-error-retry" class="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">
                    ${I18n.get('tryAgain')}
                </button>
        ...
        // Handle buttons
        const retryBtn = document.getElementById('language-error-retry');
        ...
        if (retryBtn) {
            retryBtn.addEventListener('click', () => {
                ...
                // Re-trigger language switch
                const link = document.querySelector(`#lang-menu a[hreflang="${langCode}"], #mobile-lang-menu a[hreflang="${langCode}"]`);
                if (link) link.click();
            });
        }
    ...
    function fetchLanguageContent(url, langCode, pageState) {
        ...
        .then(html => {
            ...
            // Update STRINGS object with new language strings
